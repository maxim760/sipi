...
public class GoodsController {
    ...
    //    сделать изменение ингридиентов
    @PreAuthorize("hasAuthority('ADMIN')")
    @PostMapping("/{goodsId}/product")
    public String editGoodsProducts(RedirectAttributes attrs, GoodsEntity goodsItem, @PathVariable UUID goodsId, Model model) {
        try {
            goodsService.editProducts(goodsId, goodsItem.getProducts());
            attrs.addFlashAttribute("message", "Изменены ингридиенты");
            return "redirect:/goods";
        } catch (Exception e) {
            attrs.addFlashAttribute("errorTitle", "Ошибка при изменении скидки");
            attrs.addFlashAttribute("errorMessage", e.getMessage());
